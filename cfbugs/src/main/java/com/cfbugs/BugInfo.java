...
public class BugInfo {
	...
	public static class BugInfoBuilder{
		...
		public BugInfo build(CFParsedStatement expression,final Element elem) {
			...
			int elemColumn = 0;
			if(elem != null){
				elemLine = elem.getSource().getRow(elem.getBegin())-1;
				elemColumn = elem.getSource().getColumn(elem.getBegin())-1;
			}
			bugInfo.setLine(elemLine + expression.getLine());
			bugInfo.setColumn(elemColumn + expression.getColumn());
			return bugInfo;
		}
	}
