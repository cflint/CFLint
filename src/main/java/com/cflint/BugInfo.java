...
public class BugInfo implements Comparable<BugInfo> {
	...
	public static class BugInfoBuilder {
		...
		public BugInfo build(CFParsedStatement expression, final Element elem) {
			...
			bugInfo.setLine(elemLine
					+ Math.max(expression == null ? 0
							: expression.getLine() - 1, 0));
			bugInfo.setColumn(elemColumn
					+ Math.max(expression == null ? 0
							: expression.getColumn() - 1, 0));
			doMessageText(elem);
			return bugInfo;
		}
	...
	public String toString() {
		return "BugInfo [filename=" + filename + ", line=" + line + ", column="
				+ column + ", message=" + message + ", messageCode="
				+ messageCode + ", expression=" + expression + "]";
	}
