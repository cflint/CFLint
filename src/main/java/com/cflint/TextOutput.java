...
public class TextOutput {
	...
	public void output(final BugList bugList, final Writer sb,
		...
		for (final Entry<String, List<BugInfo>> bugEntry : bugList.getBugList()
			...
			for (final BugInfo bugInfo : bugEntry.getValue()) {
				...
				sb.append(newLine).append("\tFile:")
						.append(bugInfo.getFilename());
				sb.append(newLine)
						.append("\tColumn:")
						.append(Integer.valueOf(bugInfo.getColumn()).toString());
				sb.append(newLine).append("\tLine:")
						.append(Integer.valueOf(bugInfo.getLine()).toString());
				sb.append(newLine).append("\t\tMessage:")
						.append(bugInfo.getMessage());
				sb.append(newLine).append("\t\tVariable:'")
						.append(bugInfo.getVariable());
				sb.append("' in function: ").append(bugInfo.getFunction());
				sb.append(newLine).append("\t\tExpression:")
						.append(bugInfo.getExpression());
				sb.append(newLine);
			}
		}
		...
		if (showStats) {
			sb.append(newLine).append(newLine)
					.append("Issue counts:" + counts.noBugTypes());
			...
			sb.append(newLine).append(newLine)
					.append("Total issues:" + counts.noBugs());

			for (String severity : BugCounts.levels) {
				if (counts.getSeverity(severity) > 0) {
					sb.append(newLine).append(
							"Total " + severity.toLowerCase() + "s:"
									+ counts.getSeverity(severity));
				}
			}
		}
