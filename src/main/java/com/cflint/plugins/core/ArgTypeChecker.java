...
public class ArgTypeChecker extends CFLintScannerAdapter {
				...
				if (variableType == null) {
					...
					bugs.add(new BugInfo.BugInfoBuilder()
							...
							.setMessage(
									"Argument "
											+ name
											+ " is any. Please change to be the correct type.")
							.setVariable(name).build());
				}
			}
		}
	...
	public void element(final Element element, final Context context,
		...
		if (element.getName().equals("cfargument")) {
			...
			if (variableType == null) {
				bugs.add(new BugInfo.BugInfoBuilder().setLine(begLine)
						.setMessageCode("ARG_TYPE_MISSING")
						.setSeverity(severity)
						.setFilename(context.getFilename())
						.setFunction(context.getFunctionName())
