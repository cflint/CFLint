...
public class ArgumentNameChecker extends CFLintScannerAdapter {
    ...
    public void expression(final CFScriptStatement expression, final Context context, final BugList bugs) {
        if (expression instanceof CFFuncDeclStatement) {
            ...
            for (final CFFunctionParameter argument : function.getFormals()) {
                checkNameForBugs(context, argument.getName(), context.getFilename(), context.getFunctionName(), bugs,
                        context.messageBuilder(this).at(argument));
            }
        }
    }
    ...
    public void element(final Element element, final Context context, final BugList bugs) {
        if (element.getName().equals(CF.CFARGUMENT)) {
            ...
            if (name != null && name.length() > 0) {
                checkNameForBugs(context, name, context.getFilename(), context.getFunctionName(), bugs,
                        context.messageBuilder(this).at(element));
            } else {
     ...
     * valid name - maximum number of words in a camel case name - name prefixes
     * to avoid - name postfixes to avoid
     *
     ...
     *
     * Bad arguments include: - Invalid names (contains an invalid character,
     * ends in a number, not camelCase or does not use underscores) - Names all
     * in upper case - Names that are too short - Names that are too long -
     * Names that are too wordy - Names that look like temporary variables -
    ...
    public void checkNameForBugs(final Context context, final String argument, final String filename,
            final String functionName, final BugList bugs, final MessageBuilder msgBuilder) {
