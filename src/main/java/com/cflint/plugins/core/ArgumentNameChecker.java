...
public class ArgumentNameChecker extends CFLintScannerAdapter {
	...
	public void expression(final CFScriptStatement expression,
		...
		if (expression instanceof CFFuncDeclStatement) {
			...
			for (final CFFunctionParameter argument : function.getFormals()) {
				checkNameForBugs(argument.getName(), context.getFilename(),
	...
	public void element(final Element element, final Context context,
		...
		if (element.getName().equals("cfargument")) {
			...
			checkNameForBugs(name, context.getFilename(),
					context.getFunctionName(), lineNo, bugs);
		}
	}

	public void checkNameForBugs(String argument, String filename,
		...
		if (name.isInvalid(argument)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("ARGUMENT_INVALID_NAME")
		...
		if (name.isUpperCase(argument)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("ARGUMENT_ALLCAPS_NAME")
					.setSeverity(severity)
					.setFilename(filename)
					.setFunction(functionName)
					.setMessage(
		...
		if (name.tooShort(argument)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("ARGUMENT_TOO_SHORT")
					.setSeverity(severity)
					.setFilename(filename)
					.setFunction(functionName)
					.setMessage(
							"Argument " + argument + " should be longer than "
									+ minArgLength + " characters.")
					.setVariable(argument).build());
		}
		if (name.tooLong(argument)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("ARGUMENT_TOO_LONG")
					.setSeverity(severity)
					.setFilename(filename)
					.setFunction(functionName)
					.setMessage(
							"Argument " + argument + " should be shorter than "
									+ maxArgLength + " characters.")
					.setVariable(argument).build());
		}
		if (!name.isUpperCase(argument) && name.tooWordy(argument)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("ARGUMENT_TOO_WORDY")
					.setSeverity(severity)
					.setFilename(filename)
					.setFunction(functionName)
					.setMessage(
		...
		if (name.isTemporary(argument)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("ARGUMENT_IS_TEMPORARY")
					.setSeverity(severity)
					.setFilename(filename)
					.setFunction(functionName)
					.setMessage(
		...
		if (name.hasPrefixOrPostfix(argument)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("ARGUMENT_HAS_PREFIX_OR_POSTFIX")
					.setSeverity(severity)
					.setFilename(filename)
					.setFunction(functionName)
					.setMessage(
							"Argument has prefix or postfix " + argument
									+ " and could be named better.")
					.setVariable(argument).build());
		}
	}
}