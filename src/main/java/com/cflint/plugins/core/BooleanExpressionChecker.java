...
public class BooleanExpressionChecker extends CFLintScannerAdapter {
	...
	protected boolean hasExplicitBooleanCheck(final String code) {
		return code.contains("== true") || code.contains("eq true")
				|| code.contains("is true") || code.contains("!= true")
				|| code.contains("== false") || code.contains("eq false")
				|| code.contains("is false") || code.contains("!= false");
	}

	public void booleanExpression(final int lineNo, final Context context,
			final BugList bugs) {
		bugs.add(new BugInfo.BugInfoBuilder()
				.setLine(lineNo)
				.setMessageCode("EXPLICIT_BOOLEAN_CHECK")
				.setSeverity(severity)
				.setFilename(context.getFilename())
				.setMessage(
						"Explicit check of boolean expession at " + lineNo
								+ " is not needed.").build());
	}
