...
public class CFCompareVsAssignChecker extends CFLintScannerAdapter {
	...
	List<Integer> TOKENS = Arrays.asList(CFSCRIPTLexer.EQUALSEQUALSOP,
			CFSCRIPTLexer.LT, CFSCRIPTLexer.LTE, CFSCRIPTLexer.GT,
			CFSCRIPTLexer.GTE, CFSCRIPTLexer.OR, CFSCRIPTLexer.OROPERATOR,
			CFSCRIPTLexer.EQV, CFSCRIPTLexer.XOR, CFSCRIPTLexer.AND,
			CFSCRIPTLexer.ANDOPERATOR, CFSCRIPTLexer.EQ, CFSCRIPTLexer.NEQ,
	...
	public void expression(final CFScriptStatement expression,
		...
		if (expression instanceof CFExpressionStatement) {
			...
			if (exprStatement.getExpression() instanceof CFBinaryExpression) {
				CFBinaryExpression binaryExpression = (CFBinaryExpression) exprStatement
						.getExpression();
				if (TOKENS.contains(binaryExpression.getToken().getType())) {
					context.addMessage("COMPARE_INSTEAD_OF_ASSIGN",
							binaryExpression.getToken().getText());
				}
			}
		}
	}

	protected void noNeedtoUseCreateObject(final int lineNo,
			final Context context, final BugList bugs) {
		bugs.add(new BugInfo.BugInfoBuilder()
				.setLine(lineNo)
				.setMessageCode("AVOID_USING_CREATEOBJECT")
				.setSeverity(severity)
				.setFilename(context.getFilename())
				.setMessage(
						"CreateObject statement at line "
								+ lineNo
								+ ". Use createObject(path_to_component) or even better new path_to_component().")
				.build());
	}
}