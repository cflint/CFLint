...
public class ComponentNameChecker extends CFLintScannerAdapter {
	...
	public void checkNameForBugs(String component, String filename, BugList bugs) {
		...
		if (getParameter("MinLength") != null) {
			try {
				minComponentLength = Integer
						.parseInt(getParameter("MinLength"));
			} catch (Exception e) {
			}
		}
		...
		if (getParameter("MaxLength") != null) {
			try {
				maxComponentLength = Integer
						.parseInt(getParameter("MaxLength"));
			} catch (Exception e) {
			}
		}
		...
		ValidName name = new ValidName(minComponentLength, maxComponentLength,
				maxComponentWords);
		...
		if (name.isInvalidComponent(component)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("COMPONENT_INVALID_NAME")
		...
		if (name.isUpperCase(component)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("COMPONENT_ALLCAPS_NAME")
					.setSeverity(severity)
					.setFilename(filename)
					.setMessage(
							"Component name " + component
									+ " should not be all upper case.").build());
		}
		if (name.tooShort(component)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("COMPONENT_TOO_SHORT")
					.setSeverity(severity)
					.setFilename(filename)
					.setMessage(
							"Component name " + component
									+ " should be longer than "
									+ minComponentLength + " characters.")
					.build());
		}
		if (name.tooLong(component)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("COMPONENT_TOO_LONG")
					.setSeverity(severity)
					.setFilename(filename)
					.setMessage(
							"Component name " + component
									+ " should be shorter than "
									+ maxComponentLength + " characters.")
					.build());
		}
		if (!name.isUpperCase(component) && name.tooWordy(component)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("COMPONENT_TOO_WORDY")
					.setSeverity(severity)
					.setFilename(filename)
					.setMessage(
							"Component name "
		...
		if (name.isTemporary(component)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("COMPONENT_IS_TEMPORARY")
					.setSeverity(severity)
					.setFilename(filename)
					.setMessage(
							"Component name " + component
									+ " could be named better.").build());
		}
		if (name.hasPrefixOrPostfix(component)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("COMPONENT_HAS_PREFIX_OR_POSTFIX")
					.setSeverity(severity)
					.setFilename(filename)
					.setMessage(
							"Component name "
									+ component
									+ " has prefix or postfix and could be named better.")
					.build());
		}
	}
}