...
public class LiteralChecker extends CFLintScannerAdapter {
	...
	public void expression(final CFExpression expression,
		...
		if (expression instanceof CFLiteral) {
			...
			if (warningScope == null || warningScope.equals("global")) {
				...
				literalCount(name, lineNo, functionListerals, false, context,
						bugs);
			}
		}
	}
	...
	protected void literalCount(final String name, final int lineNo,
			Map<String, Integer> literals, boolean global,
			final Context context, final BugList bugs) {
		int count = 1;
		...
		if (count > threshold
				&& (warningThreshold == -1 || (count - threshold) <= warningThreshold)) {
			if (global) {
	...
	protected boolean isCommon(final String name) {
		return name.equals("1") || name.equals("0") || name.equals("")
				|| name.equals("true") || name.equals("false");
	}

	public void magicLocalValue(final String name, final int lineNo,
			final Context context, final BugList bugs) {
		bugs.add(new BugInfo.BugInfoBuilder()
				.setLine(lineNo)
				.setMessageCode("LOCAL_LITERAL_VALUE_USED_TOO_OFTEN")
	...
	public void magicGlobalValue(final String name, final int lineNo,
			final Context context, final BugList bugs) {
		bugs.add(new BugInfo.BugInfoBuilder()
				.setLine(lineNo)
				.setMessageCode("GLOBAL_LITERAL_VALUE_USED_TOO_OFTEN")
				.setSeverity(severity)
				.setFilename(context.getFilename())
				.setMessage(
						"Literal "
