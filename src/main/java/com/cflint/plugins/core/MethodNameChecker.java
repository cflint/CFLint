...
public class MethodNameChecker extends CFLintScannerAdapter {
	...
	public void expression(final CFScriptStatement expression,
		...
		if (expression instanceof CFFuncDeclStatement) {
			...
			checkNameForBugs(context.getFunctionName(), context.getFilename(),
	...
	public void checkNameForBugs(String method, String filename,
			String functionName, int line, BugList bugs) {
		...
		ValidName name = new ValidName(minMethodLength, maxMethodLength,
				maxMethodWords);

		if (name.isInvalid(method)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("METHOD_INVALID_NAME")
					...
					.setMessage(
							"Method name "
									+ method
									+ " is not a valid name. Please use CamelCase or underscores.")
					.build());
		}
		if (name.isUpperCase(method)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("METHOD_ALLCAPS_NAME")
					.setSeverity(severity)
					.setFilename(filename)
					.setFunction(functionName)
					.setMessage(
							"Method name " + method
									+ " should not be upper case.").build());
		}
		if (name.tooShort(method)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("METHOD_TOO_SHORT")
					.setSeverity(severity)
					.setFilename(filename)
					.setFunction(functionName)
					.setMessage(
							"Method name " + method + " should be longer than "
									+ minMethodLength + " characters.").build());
		}
		if (name.tooLong(method)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("METHOD_TOO_LONG")
					.setSeverity(severity)
					.setFilename(filename)
					.setFunction(functionName)
					.setMessage(
							"Method name " + method
									...
									+ maxMethodLength + " characters.").build());
		}
		if (!name.isUpperCase(method) && name.tooWordy(method)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("METHOD_TOO_WORDY")
					.setSeverity(severity)
					.setFilename(filename)
					.setFunction(functionName)
					.setMessage(
							"Method name "
									+ method
									+ " is too wordy, can you think of a more concise name?")
					.build());
		}
		if (name.isTemporary(method)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("METHOD_IS_TEMPORARY")
					.setSeverity(severity)
					.setFilename(filename)
					.setFunction(functionName)
					.setMessage(
							"Method name " + method + " could be named better.")
					.build());
		}
		if (name.hasPrefixOrPostfix(method)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setLine(line)
					.setMessageCode("METHOD_HAS_PREFIX_OR_POSTFIX")
					.setSeverity(severity)
					.setFilename(filename)
					.setFunction(functionName)
					.setMessage(
							"Method name has prefix or postfix " + method
									+ " and could be named better.").build());
		}
	}
}