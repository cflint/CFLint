	...
	public void element(final Element element, final Context context,
		...
		if (tagName.equals("cfquery")) {
			...
			if (queryGuts.contains(selectStar)) {
				...
				bugs.add(new BugInfo.BugInfoBuilder().setLine(beginLine)
						.setMessageCode(messageCode).setSeverity(severity)
						.setFilename(context.getFilename()).setMessage(message)
						.build());
			}
		}
	}
