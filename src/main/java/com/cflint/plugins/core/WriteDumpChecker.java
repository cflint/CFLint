...
public class WriteDumpChecker extends CFLintScannerAdapter {
	...
	public void expression(final CFExpression expression,
		...
		if (expression instanceof CFFunctionExpression) {
			...
			if (functionExpression.getFunctionName().equals("writeDump")) {
				final int lineNo = functionExpression.getLine()
						+ context.startLine() - 1;
	...
	protected void writeDump(final int lineNo, final Context context,
			final BugList bugs) {
		bugs.add(new BugInfo.BugInfoBuilder()
				.setLine(lineNo)
				.setMessageCode("AVOID_USING_WRITEDUMP")
				.setSeverity(severity)
				.setFilename(context.getFilename())
				.setMessage(
						"WriteDump statement at line " + lineNo
								+ ". Avoid using writeDump in production code.")
				.build());
	}
}