...
public class CFLintFilter {
	...
	public static CFLintFilter createFilter(boolean verbose) throws IOException {
		String data = null;
		try {
			final InputStream is = CFLintFilter.class
					.getResourceAsStream("/cflintexclude.json");
			if (is == null) {
				if (verbose) {
			...
			if (verbose) {
				final URL url = CFLintFilter.class
						.getResource("/cflintexclude.json");
	...
	public boolean include(final BugInfo bugInfo) {
		if (includeCodes != null
				&& !includeCodes.contains(bugInfo.getMessageCode())) {
			return false;
		}
		if (excludeCodes != null
				&& excludeCodes.contains(bugInfo.getMessageCode())) {
			return false;
		}
		if (data != null) {
			for (final Map<String, ?> item : data) {
				...
				if (item.containsKey("file")) {
					if (!bugInfo.getFilename().matches(
							item.get("file").toString())) {
						continue;
					} else if (verbose) {
						System.out.println("Exclude matched file "
								+ bugInfo.getFilename());
					}
				}
				if (item.containsKey("code")) {
					if (!bugInfo.getMessageCode().matches(
						...
						System.out.println("Exclude matched message code "
								+ bugInfo.getMessageCode());
					}
				}
				if (item.containsKey("function")) {
					if (bugInfo.getFunction() == null
							|| !bugInfo.getFunction().matches(
									item.get("function").toString())) {
						continue;
					} else if (verbose) {
						System.out.println("Exclude matched function name "
								+ bugInfo.getFunction());
					}
				}
				if (item.containsKey("variable")) {
					if (bugInfo.getVariable() == null
							|| !bugInfo.getVariable().matches(
									item.get("variable").toString())) {
						continue;
					} else if (verbose) {
						System.out.println("Exclude matched variable name "
								+ bugInfo.getVariable());
					}
				}
				if (item.containsKey("line")) {
					if (bugInfo.getLine() > 0
							|| !new Integer(bugInfo.getLine()).toString()
									.matches(item.get("line").toString())) {
						continue;
					} else if (verbose) {
						System.out.println("Exclude matched line "
								+ bugInfo.getLine());
					}
				}
				if (item.containsKey("expression")) {
					if (bugInfo.getExpression() == null
							|| !bugInfo.getExpression().matches(
									item.get("expression").toString())) {
						continue;
					} else if (verbose) {
						System.out.println("Exclude matched expression "
